    ...
    <Formik @on-submit="handleNewVote" submit-label="Créer" :initial-values="voteForm">
      ...
      <template v-slot:submit-button>
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary">{{$store.state.voteActif ? 'Valider' : 'Créer'}}</button>
        </div>
      </template>
    </Formik>
  </Modal>
...
export default {
  ...
  // updated() {
  //   if (this.$store.state.voteActif) {
  //     this.voteForm.title = this.$store.state.voteActif.title;
  //     this.voteForm.desc = this.$store.state.voteActif.desc;

  //     this.$store.state.voteActif = null;
  //   }
  // },
  // watch: {
  //   voteForm: async function() {
  //     if (this.$store.state.voteActif) {
  //       this.voteForm.title = this.$store.state.voteActif.title;
  //       this.voteForm.desc = this.$store.state.voteActif.desc;
  //     }
  ...
  methods: {
    ...
    handleNewVote: function() {
      ...
      if (this.$store.state.voteActif) {
        ...
        this.$store.dispatch("patchVote", data).then(() => {
          // on ferme la modal
          this.onCancel();
        });
      } else {
